...
class DamageCubit extends Cubit<DamageState> {
  ...
  Future<void> loadProducts() async {
    emit(state.copyWith(status: DamageStatus.loading));
    try {
      final products = await _productRepository.getProducts();
      emit(state.copyWith(status: DamageStatus.success, products: products));
    } catch (e) {
      emit(state.copyWith(status: DamageStatus.failure, errorMessage: 'فشل تحميل المنتجات'));
    }
  }
  ...
  Future<void> submitBatch(String notes) async {
    ...
    try {
      ...
      emit(state.copyWith(status: DamageStatus.submitted, cart: {}));
    } catch (e) {
      emit(state.copyWith(
        status: DamageStatus.failure,
        errorMessage: e.toString().contains('المخزون') ? e.toString() : 'فشل تسجيل العملية',
      ));
    }
  }
}
